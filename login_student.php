...
{    
	...
	$data=mysqli_query($link,"SELECT * FROM stu_log WHERE user_name='$name' ");
   /* $data1=mysqli_query($link,"SELECT * FROM tbl_students WHERE roll_no='$name' AND (titleA IS NOT NULL OR titleB IS NOT NULL OR titleC IS NOT NULL OR titleD IS NOT NULL OR titleE IS NOT NULL OR titleF IS NOT NULL OR titleG IS NOT NULL OR titleH IS NOT NULL) ");
	...
	if ($row>0) {
		...
		{ 
            session_start();
            $_SESSION['roll_no']=$arr['user_name'];
            $roll_no=$_SESSION['roll_no'];
            $insert1="SELECT * FROM tbl_student WHERE roll_no='$roll_no'";
            $data1=mysqli_query($link,$insert1);
            $row=mysqli_num_rows($link,$data1);
            if(!$row)
            {
                $insert2="INSERT INTO tbl_students(roll_no) VALUES('$roll_no')";
